...
def _transcribe_with_whispercpp(media_path: str, language: Optional[str]) -> Dict:
    """Transcribe via the native whisper.cpp CLI, avoiding Python inference runtimes."""
    command = shutil.which(WHISPER_CPP_COMMAND)
    if not command:
        raise RuntimeError("whisper.cpp is required. Install it with: brew install whisper-cpp")
    model = Path(WHISPER_CPP_MODEL).expanduser()
    if not model.is_file():
        raise RuntimeError(
            f"whisper.cpp model not found: {model}. Download a multilingual GGML model and set WHISPER_CPP_MODEL."
        )
    with tempfile.TemporaryDirectory(prefix="shorts-whisper-") as temp_dir:
        wav_path = Path(temp_dir) / "audio.wav"
        output_prefix = Path(temp_dir) / "transcript"
        subprocess.run(
            ["ffmpeg", "-y", "-i", media_path, "-vn", "-ac", "1", "-ar", "16000", str(wav_path)],
            check=True,
            stdout=subprocess.DEVNULL,
            stderr=subprocess.DEVNULL,
        )
        # The CLI may auto-select Metal on macOS. CPU is the portable default
        # for unattended/headless jobs; users can remove this flag locally if
        # they have a verified Metal setup.
        command_args = [command, "-m", str(model), "-f", str(wav_path), "-l", language or "auto", "-osrt", "-of", str(output_prefix), "-np", "-ng"]
        print(f"[transcribe/whisper.cpp] model={model.name}", flush=True)
        subprocess.run(command_args, check=True)
        srt_path = output_prefix.with_suffix(".srt")
        if not srt_path.is_file():
            raise RuntimeError("whisper.cpp did not produce an SRT transcript")
        return _load_srt_cache(srt_path)
...
def transcribe_local(media_path: str, language: Optional[str] = None) -> Dict:
    ...
    if LOCAL_TRANSCRIBER_BACKEND in {"whispercpp", "whisper.cpp"}:
        transcript = _transcribe_with_whispercpp(media_path, language)
        cache_path = _write_srt_cache(media_path, transcript)
        print(f"[transcribe/local] wrote cache: {cache_path}", flush=True)
        return transcript
    if LOCAL_TRANSCRIBER_BACKEND != "faster-whisper":
        raise RuntimeError("LOCAL_TRANSCRIBER_BACKEND must be 'whispercpp' or 'faster-whisper'.")
